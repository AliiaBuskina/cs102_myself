...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    row, col = pos
    # Удаляем значения, которые уже присутствуют в строке
    for i in range(9):
        if grid[row][i] in values:
            values.remove(grid[row][i])
    # Удаляем значения, которые уже присутствуют в столбце
    for i in range(9):
        if grid[i][col] in values:
            values.remove(grid[i][col])
    # Удаляем значения, которые уже присутствуют в блоке
    block_row = (row // 3) * 3
    block_col = (col // 3) * 3
